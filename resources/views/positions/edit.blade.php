<x-app-layout>
    ...
    <h1 class="text-3xl font-bold mb-6">Edit Jabatan</h1>

    <form action="{{ route('positions.update', $position) }}" method="POST" class="bg-white shadow-md rounded-lg p-6">
        @csrf
        @method('PUT')

        <div class="mb-4">
            <label for="name" class="block text-sm font-semibold mb-2">Nama Jabatan</label>
            <input 
                type="text" 
                name="name" 
                id="name" 
        ...
        <div class="flex gap-2">
            ...
            <a href="{{ route('positions.index') }}" class="bg-gray-400 hover:bg-gray-500 text-white px-4 py-2 rounded">
                Batal
            </a>
        </div>
    </form>
